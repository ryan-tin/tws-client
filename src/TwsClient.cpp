...
// TEST: do action by setting state;
// TODO: how to wait for response?
TwsClient::TwsClient(State action)
    : m_osSignal(2000) // 2-seconds timeout
      ,
      m_pClient(new EClientSocket(this, &m_osSignal)), m_state(ST_CONNECT),
      m_sleepDeadline(0), m_orderId(0), m_extraAuth(false) {
  const char *host = "";
  const int port = 7497; // NOTE: 7947 for simulation -1 for live?
  const int clientId = 0;
  const char *connectOptions = "+PACEAPI"; // fixes pacing

  unsigned attempt = 0;
  // printf("Start of C++ Socket Client Test %u\n", attempt);

  this->m_state = action; // set state to do a single action
...
bool TwsClient::connect(const char *host, int port, int clientId) {
  ...
  if (bRes) {
    ...
    //! [ereader]
    // automatically start the ereader thread after connected
...
void TwsClient::setOptionalCapabilities(
...
void TwsClient::pnlSingleOperation() {
  //! [reqpnlsingle]
...
void TwsClient::tickDataOperation() {
  ...
  //! [reqmktdata]
  m_pClient->reqMktData(1001, ContractBuilder::StockComboContract(), "", false,
                        false, TagValueListSPtr());
  m_pClient->reqMktData(1002, ContractBuilder::OptionWithLocalSymbol(), "",
                        false, false, TagValueListSPtr());
  //! [reqmktdata]
  //! [reqmktdata_snapshot]
  m_pClient->reqMktData(1003, ContractBuilder::FutureComboContract(), "", true,
                        false, TagValueListSPtr());
  //! [reqmktdata_snapshot]
  ...
  m_pClient->reqMktData(1013, ContractBuilder::USStock(), "", false, true,
  TagValueListSPtr());
  //! [regulatorysnapshot]
  */
  ...
  m_pClient->reqMktData(1004, ContractBuilder::USStockAtSmart(), "233,236",
                        false, false, TagValueListSPtr());
  //! [reqmktdata_genticks]
  ...
  // error
  m_pClient->reqMktData(1005, ContractBuilder::USStock(), "mdoff,292:BZ", false,
                        false, TagValueListSPtr());
  m_pClient->reqMktData(1006, ContractBuilder::USStock(), "mdoff,292:BT", false,
                        false, TagValueListSPtr());
  m_pClient->reqMktData(1007, ContractBuilder::USStock(), "mdoff,292:FLY",
                        false, false, TagValueListSPtr());
  m_pClient->reqMktData(1008, ContractBuilder::USStock(), "mdoff,292:DJ-RT",
                        false, false, TagValueListSPtr());
  //! [reqmktdata_contractnews]
  //! [reqmktdata_broadtapenews]
  m_pClient->reqMktData(1009, ContractBuilder::BTbroadtapeNewsFeed(),
                        "mdoff,292", false, false, TagValueListSPtr());
  m_pClient->reqMktData(1010, ContractBuilder::BZbroadtapeNewsFeed(),
                        "mdoff,292", false, false, TagValueListSPtr());
  m_pClient->reqMktData(1011, ContractBuilder::FLYbroadtapeNewsFeed(),
                        "mdoff,292", false, false, TagValueListSPtr());
  //! [reqmktdata_broadtapenews]

  //! [reqoptiondatagenticks]
  // Requesting data for an option contract will return the greek values
  m_pClient->reqMktData(1013, ContractBuilder::USOptionContract(), "", false,
                        false, TagValueListSPtr());
  //! [reqoptiondatagenticks]
  ...
  // interest
  m_pClient->reqMktData(1014, ContractBuilder::SimpleFuture(), "mdoff,588",
                        false, false, TagValueListSPtr());
  //! [reqfuturesopeninterest]
  ...
  // flag
  m_pClient->reqMktData(1015, ContractBuilder::SimpleFuture(), "", false, false,
                        TagValueListSPtr());
  //! [reqpreopenbidask]

  //! [reqavgoptvolume]
  // Requesting data for a stock will return the average option volume
  m_pClient->reqMktData(1016, ContractBuilder::USStockAtSmart(), "mdoff,105",
                        false, false, TagValueListSPtr());
  //! [reqavgoptvolume]

  //! [reqetfticks]
  // Requesting data for an ETF will return the ETF ticks
  m_pClient->reqMktData(1017, ContractBuilder::etf(), "mdoff,577,614,623",
                        false, false, TagValueListSPtr());
  //! [reqetfticks]

  //! [reqmktdatacrypto]
  m_pClient->reqMktData(1018, ContractBuilder::CryptoContract(), "", false,
                        false, TagValueListSPtr());
  //! [reqmktdatacrypto]

  //! [IPOPrice]
  m_pClient->reqMktData(1019, ContractBuilder::StockWithIPOPrice(), "mdoff,586",
                        false, false, TagValueListSPtr());
  //! [IPOPrice]

  //! [yieldbidask]
  m_pClient->reqMktData(1020, ContractBuilder::Bond(), "", false, false,
                        TagValueListSPtr());
  //! [yieldbidask]
...
void TwsClient::tickOptionComputationOperation() {
  /*** Requesting real time market data ***/
  std::this_thread::sleep_for(std::chrono::seconds(1));
  ...
  //! [reqmktdata]
  m_pClient->reqMktData(2001, ContractBuilder::OptionWithLocalSymbol(), "",
                        false, false, TagValueListSPtr());
  //! [reqmktdata]
...
void TwsClient::delayedTickDataOperation() {
  /*** Requesting delayed market data ***/

  //! [reqmktdata_delayedmd]
  m_pClient->reqMarketDataType(4); // send delayed-frozen (4) market data type
  m_pClient->reqMktData(1013, ContractBuilder::HKStk(), "", false, false,
                        TagValueListSPtr());
  m_pClient->reqMktData(1014, ContractBuilder::USOptionContract(), "", false,
                        false, TagValueListSPtr());
  //! [reqmktdata_delayedmd]
...
void TwsClient::marketDepthOperations() {
  /*** Requesting the Deep Book ***/
  //! [reqmarketdepth]
  m_pClient->reqMktDepth(2001, ContractBuilder::EurGbpFx(), 5, false,
                         TagValueListSPtr());
  //! [reqmarketdepth]
  std::this_thread::sleep_for(std::chrono::seconds(2));
  ...
  /*** Requesting the Deep Book ***/
  //! [reqmarketdepth]
  m_pClient->reqMktDepth(2002, ContractBuilder::EuropeanStock(), 5, true,
                         TagValueListSPtr());
  //! [reqmarketdepth]
...
void TwsClient::realTimeBars() {
  /*** Requesting real time bars ***/
  //! [reqrealtimebars]
  m_pClient->reqRealTimeBars(3001, ContractBuilder::EurGbpFx(), 5, "MIDPOINT",
...
// NOTE: SAMPLE: do not modify, play with the other one!
// void TwsHistoricalDataClient::historicalDataRequests()
// {
...
//     char queryTime[80];
//
// 	std::time(&rawtime); // get current time
// #if defined(IB_WIN32)
//     gmtime_s(&timeinfo, &rawtime);
// #else
//     gmtime_r(&rawtime, &timeinfo); // convert time since epoch to utc time
//     (tm format)
// #endif
//     std::strftime(queryTime, sizeof queryTime, "%Y%m%d-%H:%M:%S", &timeinfo);
//     // convert to string
//
// 	m_pClient->reqHistoricalData(4001, ContractBuilder::EurGbpFx(),
// queryTime, "1 M", "1 day", "MIDPOINT", 1, 1, false, TagValueListSPtr());
// 	m_pClient->reqHistoricalData(4002, ContractBuilder::EuropeanStock(),
// queryTime, "10 D", "1 min", "TRADES", 1, 1, false, TagValueListSPtr());
// 	m_pClient->reqHistoricalData(4003, ContractBuilder::USStockAtSmart(),
// queryTime, "1 M", "1 day", "SCHEDULE", 1, 1, false, TagValueListSPtr());
// 	//! [reqhistoricaldata]
// 	std::this_thread::sleep_for(std::chrono::seconds(2));
...
void TwsClient::historicalDataRequests() {
  /*** Requesting historical data ***/
  //! [reqhistoricaldata]
  std::time_t rawtime;
  std::tm timeinfo;
...
#else
  gmtime_r(&rawtime,
           &timeinfo); // convert time since epoch to utc time (tm format)
#endif
  std::strftime(queryTime, sizeof queryTime, "%Y%m%d-%H:%M:%S",
                &timeinfo); // convert to string

  // TEST: get last 1 year daily data for USDHKD
  // can i get further back than the last year if i change the queryTime?
  ...
  // std::this_thread::sleep_for(std::chrono::seconds(2));
  /*** Canceling historical data requests ***/
  // m_pClient->cancelHistoricalData(4001); // NOTE: i dont think this needs to
  // be cancelled, its just an example m_pClient->cancelHistoricalData(4002);
  // m_pClient->cancelHistoricalData(4003);
}

void TwsClient::optionsOperations() {
  //! [reqsecdefoptparams]
  ...
  m_pClient->calculateImpliedVolatility(
      5001, ContractBuilder::OptionWithLocalSymbol(), 0.5, 55,
      TagValueListSPtr());
  //! [calculateimpliedvolatility]
  ...
  m_pClient->calculateOptionPrice(5002,
                                  ContractBuilder::OptionWithLocalSymbol(), 0.6,
                                  55, TagValueListSPtr());
  //! [calculateoptionprice]
  ...
  //! [exercise_options]
  //** Exercising options ***
  m_pClient->exerciseOptions(5003, ContractBuilder::OptionWithTradingClass(), 1,
...
void TwsClient::contractOperations() {
  m_pClient->reqContractDetails(209, ContractBuilder::EurGbpFx());
  std::this_thread::sleep_for(std::chrono::seconds(2));
  //! [reqcontractdetails]
  m_pClient->reqContractDetails(210, ContractBuilder::OptionForQuery());
  m_pClient->reqContractDetails(212, ContractBuilder::IBMBond());
  m_pClient->reqContractDetails(213, ContractBuilder::IBKRStk());
  m_pClient->reqContractDetails(214, ContractBuilder::Bond());
  m_pClient->reqContractDetails(215, ContractBuilder::FuturesOnOptions());
  m_pClient->reqContractDetails(216, ContractBuilder::SimpleFuture());
  m_pClient->reqContractDetails(219, ContractBuilder::Fund());
  m_pClient->reqContractDetails(220, ContractBuilder::USStock());
  m_pClient->reqContractDetails(221, ContractBuilder::USStockAtSmart());
  //! [reqcontractdetails]

  //! [reqcontractdetailsnews]
  m_pClient->reqContractDetails(211, ContractBuilder::NewsFeedForQuery());
  //! [reqcontractdetailsnews]

  //! [reqcontractdetailscrypto]
  m_pClient->reqContractDetails(217, ContractBuilder::CryptoContract());
  //! [reqcontractdetailscrypto]

  //! [reqcontractdetailsbyissuerid]
  m_pClient->reqContractDetails(218, ContractBuilder::ByIssuerId());
  //! [reqcontractdetailsbyissuerid]

  m_state = ST_CONTRACTOPERATION_ACK;
}

void TwsClient::marketScanners() {
...
void TwsClient::fundamentals() {
  /*** Requesting Fundamentals ***/
  //! [reqfundamentaldata]
  m_pClient->reqFundamentalData(8001, ContractBuilder::USStock(),
...
// NOTE: default function contents
// void TwsHistoricalDataClient::accountOperations()
// {
...
// 	//! [reqaaccountsummary]
// 	m_pClient->reqAccountSummary(9001, "All",
...
void TwsClient::orderOperations() {
  ...
  m_pClient->placeOrder(
      m_orderId++, ContractBuilder::USStock(),
      OrderSamples::LimitOrder("SELL", DecimalFunctions::stringToDecimal("1"),
                               50));
  //! [order_submission]

  // m_pClient->placeOrder(m_orderId++, ContractBuilder::OptionAtBox(),
  ...
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::FutureComboContract(),
  // OrderSamples::ComboLimitOrder("SELL",
  // DecimalFunctions::stringToDecimal("1"), 1, false));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::StockComboContract(),
  // OrderSamples::ComboMarketOrder("BUY",
  // DecimalFunctions::stringToDecimal("1"), false));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::OptionComboContract(),
  // OrderSamples::ComboMarketOrder("BUY",
  // DecimalFunctions::stringToDecimal("1"), true));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::StockComboContract(),
  // OrderSamples::LimitOrderForComboWithLegPrices("BUY",
  // DecimalFunctions::stringToDecimal("1"), std::vector<double>(10, 5), true));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::Discretionary("SELL", DecimalFunctions::stringToDecimal("1"),
  // 45, 0.5)); m_pClient->placeOrder(m_orderId++,
  // ContractBuilder::OptionAtBox(), OrderSamples::LimitIfTouched("BUY",
  // DecimalFunctions::stringToDecimal("1"), 30, 34));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::LimitOnClose("SELL", DecimalFunctions::stringToDecimal("1"),
  // 34)); m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::LimitOnOpen("BUY", DecimalFunctions::stringToDecimal("1"),
  // 35)); m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  ...
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::MarketOnOpen("BUY", DecimalFunctions::stringToDecimal("1")));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::MarketOrder("SELL", DecimalFunctions::stringToDecimal("1")));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::MarketToLimit("BUY",
  // DecimalFunctions::stringToDecimal("1")));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::OptionAtIse(),
  // OrderSamples::MidpointMatch("BUY",
  // DecimalFunctions::stringToDecimal("1")));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::Stop("SELL", DecimalFunctions::stringToDecimal("1"), 34.4));
  // m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::StopLimit("BUY", DecimalFunctions::stringToDecimal("1"), 35,
  // 33)); m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  ...
  // 35)); m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::TrailingStop("SELL", DecimalFunctions::stringToDecimal("1"),
  // 0.5, 30)); m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(),
  // OrderSamples::TrailingStopLimit("BUY",
  // DecimalFunctions::stringToDecimal("100"), 2, 5, 50));

  //! [place_midprice]
  m_pClient->placeOrder(
      m_orderId++, ContractBuilder::USStockAtSmart(),
      OrderSamples::Midprice("BUY", DecimalFunctions::stringToDecimal("1"),
                             150));
  //! [place_midprice]
  ...
  //! [order_submission]
  m_pClient->placeOrder(
      m_orderId++, ContractBuilder::CryptoContract(),
      OrderSamples::LimitOrder(
          "BUY", DecimalFunctions::stringToDecimal("0.12345678"), 3700));
  //! [order_submission]

  //! [manual_order_time]
  m_pClient->placeOrder(m_orderId++, ContractBuilder::USStockAtSmart(),
  ...
  m_pClient->cancelOrder(m_orderId - 1, OrderSamples::OrderCancelWithManualTime(
                                            "20240614-00:00:05"));
  //! [manual_order_cancel_time]

  //! [pegbest_up_to_mid_order_submission]
  m_pClient->placeOrder(m_orderId++, ContractBuilder::IBKRATSContract(),
  ...
  m_pClient->placeOrder(
      m_orderId++, ContractBuilder::IBKRATSContract(),
      OrderSamples::PegMidOrder("BUY", DecimalFunctions::stringToDecimal("100"),
  ...
  m_pClient->placeOrder(
      ...
      OrderSamples::LimitOrderWithCustomerAccount(
          "BUY", DecimalFunctions::stringToDecimal("100"), 111.11, "CustAcct"));
  //! [customer_account]

  m_state = ST_ORDEROPERATIONS_ACK;
}

void TwsClient::ocaSamples() {
  // OCA ORDER
  //! [ocasubmit]
...
void TwsClient::conditionSamples() {
  ...
  PercentChangeCondition *pctChangeCondition =
      dynamic_cast<PercentChangeCondition *>(
          OrderSamples::Percent_Change_Condition(15.0, 208813720, "SMART", true,
                                                 true));
  TimeCondition *timeCondition =
      dynamic_cast<TimeCondition *>(OrderSamples::Time_Condition(
          "20220808 10:00:00 US/Eastern", true, false));
  VolumeCondition *volumeCondition = dynamic_cast<VolumeCondition *>(
  ...
  m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(), lmt);
  //! [order_conditioning_activate]
  ...
  m_pClient->placeOrder(m_orderId++, ContractBuilder::EuropeanStock(), lmt2);
  //! [order_conditioning_cancel]

  m_state = ST_CONDITIONSAMPLES_ACK;
}

void TwsClient::bracketSample() {
  Order parent;
  Order takeProfit;
  Order stopLoss;
  //! [bracketsubmit]
  OrderSamples::BracketOrder(m_orderId++, parent, takeProfit, stopLoss, "BUY",
                             ...
                             20);
  m_pClient->placeOrder(parent.orderId, ContractBuilder::EuropeanStock(),
                        parent);
  m_pClient->placeOrder(takeProfit.orderId, ContractBuilder::EuropeanStock(),
                        takeProfit);
  m_pClient->placeOrder(stopLoss.orderId, ContractBuilder::EuropeanStock(),
                        stopLoss);
  //! [bracketsubmit]
...
void TwsClient::hedgeSample() {
  ...
  m_pClient->placeOrder(m_orderId++, ContractBuilder::EurGbpFx(), hedge);
  //! [hedgesubmit]

  m_state = ST_HEDGESAMPLES_ACK;
}

void TwsClient::testAlgoSamples() {
  //! [algo_base_order]
  ...
  m_pClient->placeOrder(m_orderId++, ContractBuilder::JefferiesContract(),
                        baseOrder);
  //! [jeff_vwap_algo]

  //! [csfb_inline_algo]
  AvailableAlgoParams::FillCSFBInlineParams(
      baseOrder, "10:00:00 US/Eastern", "16:00:00 US/Eastern", "Patient", 10,
      20, 100, "Default", false, 40, 100, 100, 35);
  m_pClient->placeOrder(m_orderId++, ContractBuilder::CSFBContract(),
                        baseOrder);
  //! [csfb_inline_algo]
...
void TwsClient::financialAdvisorOrderSamples() {
  ...
  m_pClient->placeOrder(m_orderId++, ContractBuilder::USStock(), modelOrder);
  //! [modelorder]

  m_state = ST_FAORDERSAMPLES_ACK;
}

void TwsClient::financialAdvisorOperations() {
...
void TwsClient::testDisplayGroups() {
  //! [querydisplaygroups]
...
void TwsClient::reqMatchingSymbols() {
...
void TwsClient::reqMktDepthExchanges() {
...
void TwsClient::reqNewsTicks() {
  //! [reqmktdata_ticknews]
  m_pClient->reqMktData(12001, ContractBuilder::USStockAtSmart(), "mdoff,292",
                        false, false, TagValueListSPtr());
  //! [reqmktdata_ticknews]
...
void TwsClient::reqSmartComponents() {
  static bool bFirstRun = true;

  if (bFirstRun) {
    m_pClient->reqMktData(13001, ContractBuilder::USStockAtSmart(), "", false,
                          false, TagValueListSPtr());
...
void TwsClient::reqNewsProviders() {
...
void TwsClient::reqHistoricalNews() {
...
void TwsClient::reqHeadTimestamp() {
  //! [reqHeadTimeStamp]
  m_pClient->reqHeadTimestamp(14001, ContractBuilder::EurGbpFx(), "MIDPOINT", 1,
                              1);
  //! [reqHeadTimeStamp]
  std::this_thread::sleep_for(std::chrono::seconds(1));
  //! [cancelHeadTimestamp]
...
void TwsClient::reqHistogramData() {
  //! [reqHistogramData]
  m_pClient->reqHistogramData(15001, ContractBuilder::IBMUSStockAtSmart(),
                              false, "1 weeks");
  //! [reqHistogramData]
  std::this_thread::sleep_for(std::chrono::seconds(2));
  //! [cancelHistogramData]
...
void TwsClient::rerouteCFDOperations() {
  //! [reqmktdatacfd]
  m_pClient->reqMktData(16001, ContractBuilder::USStockCFD(), "", false, false,
                        TagValueListSPtr());
  std::this_thread::sleep_for(std::chrono::seconds(1));
  m_pClient->reqMktData(16002, ContractBuilder::EuropeanStockCFD(), "", false,
                        false, TagValueListSPtr());
  std::this_thread::sleep_for(std::chrono::seconds(1));
  m_pClient->reqMktData(16003, ContractBuilder::CashCFD(), "", false, false,
                        TagValueListSPtr());
  std::this_thread::sleep_for(std::chrono::seconds(1));
  //! [reqmktdatacfd]

  //! [reqmktdepthcfd]
  m_pClient->reqMktDepth(16004, ContractBuilder::USStockCFD(), 10, false,
                         TagValueListSPtr());
  std::this_thread::sleep_for(std::chrono::seconds(1));
  m_pClient->reqMktDepth(16005, ContractBuilder::EuropeanStockCFD(), 10, false,
                         TagValueListSPtr());
  std::this_thread::sleep_for(std::chrono::seconds(1));
  m_pClient->reqMktDepth(16006, ContractBuilder::CashCFD(), 10, false,
                         TagValueListSPtr());
  std::this_thread::sleep_for(std::chrono::seconds(1));
  //! [reqmktdepthcfd]
...
void TwsClient::marketRuleOperations() {
  m_pClient->reqContractDetails(17001, ContractBuilder::IBMBond());
  m_pClient->reqContractDetails(17002, ContractBuilder::IBKRStk());
...
void TwsClient::continuousFuturesOperations() {
  m_pClient->reqContractDetails(18001, ContractBuilder::ContFut());
  ...
  m_pClient->reqHistoricalData(18002, ContractBuilder::ContFut(), queryTime,
...
void TwsClient::reqHistoricalTicks() {
  //! [reqhistoricalticks]
  m_pClient->reqHistoricalTicks(19001, ContractBuilder::IBMUSStockAtSmart(),
                                "20170621 09:38:33 US/Eastern", "", 10,
                                "BID_ASK", 1, true, TagValueListSPtr());
  m_pClient->reqHistoricalTicks(19002, ContractBuilder::IBMUSStockAtSmart(),
                                "20170621 09:38:33 US/Eastern", "", 10,
                                "MIDPOINT", 1, true, TagValueListSPtr());
  m_pClient->reqHistoricalTicks(19003, ContractBuilder::IBMUSStockAtSmart(),
                                "20170621 09:38:33 US/Eastern", "", 10,
...
void TwsClient::reqTickByTickData() {
  ...
  m_pClient->reqTickByTickData(20001, ContractBuilder::EuropeanStock(), "Last",
                               0, false);
  m_pClient->reqTickByTickData(20002, ContractBuilder::EuropeanStock(),
                               "AllLast", 0, false);
  m_pClient->reqTickByTickData(20003, ContractBuilder::EuropeanStock(),
                               "BidAsk", 0, true);
  m_pClient->reqTickByTickData(20004, ContractBuilder::EurGbpFx(), "MidPoint",
                               0, false);
  ...
  //! [reqtickbytick]
  m_pClient->reqTickByTickData(20005, ContractBuilder::EuropeanStock(), "Last",
                               10, false);
  m_pClient->reqTickByTickData(20006, ContractBuilder::EuropeanStock(),
                               "AllLast", 10, false);
  m_pClient->reqTickByTickData(20007, ContractBuilder::EuropeanStock(),
                               "BidAsk", 10, false);
  m_pClient->reqTickByTickData(20008, ContractBuilder::EurGbpFx(), "MidPoint",
                               10, true);
  //! [reqtickbytick]
...
void TwsClient::whatIfSamples() {
  /*** Placing waht-if order ***/
  //! [whatiforder]
  m_pClient->placeOrder(
      m_orderId++, ContractBuilder::BondWithCusip(),
...
void TwsClient::wshCalendarOperations() {
  //! [reqmetadata]
...
void TwsClient::rfqOperations() {
  //! [rfq_submission]
  m_pClient->placeOrder(m_orderId++, ContractBuilder::BondWithCusip(),
...
void TwsClient::nextValidId(OrderId orderId) {
  ...
  // TODO: is this function used as a callback?
  // from doces:
  // Important: The IBApi.EWrapper.nextValidID callback is commonly used to
  // indicate that the connection is completed and other messages can be sent
  // from the API client to TWS. There is the possibility that function calls
...
void TwsClient::openOrder(OrderId orderId, const Contract &contract,
  ...
  printf(
      ...
      "MinCompeteSize: %s, CompeteAgainstBestOffset: %s, MidOffsetAtWhole: %s, "
      "MidOffsetAtHalf: %s, "
      "FAGroup: %s, FAMethod: %s, CustomerAccount: %s, ProfessionalCustomer: "
      ...
      DecimalFunctions::decimalStringToDisplay(order.totalQuantity).c_str(),
      Utils::doubleMaxString(order.cashQty).c_str(),
      Utils::doubleMaxString(order.lmtPrice).c_str(),
      Utils::doubleMaxString(order.auxPrice).c_str(), orderState.status.c_str(),
      ...
      order.competeAgainstBestOffset == COMPETE_AGAINST_BEST_OFFSET_UP_TO_MID
          ? "UpToMid"
          : Utils::doubleMaxString(order.competeAgainstBestOffset).c_str(),
...
//! [openorderend]
void TwsClient::openOrderEnd() { printf("OpenOrderEnd\n"); }
//! [openorderend]
...
void TwsClient::printContractDetailsMsg(
    const ContractDetails &contractDetails) {
...
void TwsClient::printContractDetailsSecIdList(
...
void TwsClient::printContractDetailsIneligibilityReasonList(
...
void TwsClient::printBondContractDetailsMsg(
    const ContractDetails &contractDetails) {
...
void TwsClient::historicalData(TickerId reqId, const Bar &bar) {
  printf("HistoricalData. ReqId: %ld - Date: %s, Open: %s, High: %s, Low: %s, "
...
void TwsClient::deltaNeutralValidation(
...
//! [positionend]
void TwsClient::positionEnd() { printf("PositionEnd\n"); }
//! [positionend]
...
void TwsClient::securityDefinitionOptionalParameter(
...
//! [mktDepthExchanges]
void TwsClient::mktDepthExchanges(
...
#endif
  printf("News Tick. TickerId: %d, TimeStamp: %s, ProviderCode: %s, ArticleId: "
...
void TwsClient::historicalTicksBidAsk(
  ...
  for (const HistoricalTickBidAsk &tick : ticks) {
    ...
    std::cout << "Historical tick bid/ask. ReqId: " << reqId
              << ", time: " << timeStr << ", price bid: "
              << Utils::doubleMaxString(tick.priceBid).c_str()
              << ", price ask: "
              << Utils::doubleMaxString(tick.priceAsk).c_str() << ", size bid: "
              << DecimalFunctions::decimalStringToDisplay(tick.sizeBid).c_str()
              << ", size ask: "
              << DecimalFunctions::decimalStringToDisplay(tick.sizeAsk).c_str()
...
//! [historicaltickslast]
void TwsClient::historicalTicksLast(
...
#endif
  printf(
      "Tick-By-Tick. ReqId: %d, TickType: %s, Time: %s, Price: %s, Size: %s, "
...
void TwsClient::tickByTickBidAsk(int reqId, time_t time, double bidPrice,
  ...
  ctime_r(&time, timeStr);
#endif
  printf("Tick-By-Tick. ReqId: %d, TickType: BidAsk, Time: %s, BidPrice: %s, "
         "AskPrice: %s, BidSize: %s, AskSize: %s, BidPastLow: %d, AskPastHigh: "
         "%d\n",
         reqId, timeStr, Utils::doubleMaxString(bidPrice).c_str(),
...
#endif
  printf(
      "Tick-By-Tick. ReqId: %d, TickType: MidPoint, Time: %s, MidPoint: %s\n",
...
void TwsClient::completedOrder(const Contract &contract, const Order &order,
  ...
  printf(
      ...
      "CompeteAgainstBestOffset: %s, MidOffsetAtWhole: %s, MidOffsetAtHalf: "
      "%s, "
      "CustomerAccount: %s, ProfessionalCustomer: %s\n",
      Utils::intMaxString(order.permId).c_str(),
      Utils::llongMaxString(order.parentPermId).c_str(), order.account.c_str(),
      contract.symbol.c_str(), contract.secType.c_str(),
      contract.exchange.c_str(), order.action.c_str(), order.orderType.c_str(),
      DecimalFunctions::decimalStringToDisplay(order.totalQuantity).c_str(),
      Utils::doubleMaxString(order.cashQty).c_str(),
      DecimalFunctions::decimalStringToDisplay(order.filledQuantity).c_str(),
      Utils::doubleMaxString(order.lmtPrice).c_str(),
      Utils::doubleMaxString(order.auxPrice).c_str(), orderState.status.c_str(),
      orderState.completedTime.c_str(), orderState.completedStatus.c_str(),
      Utils::intMaxString(order.minTradeQty).c_str(),
      Utils::intMaxString(order.minCompeteSize).c_str(),
      order.competeAgainstBestOffset == COMPETE_AGAINST_BEST_OFFSET_UP_TO_MID
          ? "UpToMid"
...
//! [historicalSchedule]
void TwsClient::historicalSchedule(
